...
function reportemails($db, $LOCAL ) { 
...
//while ( $row3 = $result->fetch_assoc() ) { 
	...
	//echo "$mail->AddAddress(" .$row3['EmailAdd']. ");";
...
function fromemails($db, $LOCAL ) { 
$query = "SELECT * FROM  tbl_hubslics   WHERE  	SlicSort = '$LOCAL' "; 
$result = $db->query($query); 

while ( $row3 = $result->fetch_assoc() ) { 
	$from = $row3['SrtMgrEmail'];  
...
while ( $row4 = $result->fetch_assoc() ) { 
	echo "Hello " . $row4['name_first'] . " " . $row4['name_last'] . "! In order to update your email subscriptions, simply click the report that you would like to add or remove. Please select your desired SLIC if the report requires it. Thank you and have a Great Day!";
...
if($result->num_rows > 0) {
	...
	if($curcarrier==""){
 	echo  "<select name='CARRIER' id='CARRIER'> <option value = ''>---Select---</option>";
while ( $row4 = $result->fetch_assoc() ) { 
