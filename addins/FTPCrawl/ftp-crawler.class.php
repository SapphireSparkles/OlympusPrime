		...
		switch ($type) {
			...
			case 'xml':
				...
				foreach ($this->list as $key => $value) {
					...
					$dir = $xml_doc->createElement('dir', $pieces[2]);
					$subdir = $xml_doc->createElement('subdir', $pieces[3]);
					if($dircount >=5) {
					$subsubdir = $xml_doc->createElement('subsubdir', $pieces[4]);
					$mina = $pieces[4];
					}else{
					$subsubdir = $xml_doc->createElement('subsubdir', 'none');
					$mina = "none";
					}
					$minion = $pieces[2] . "/" .$pieces[3] ;
					$miniondir = $xml_doc->createElement('minion', $minion);
